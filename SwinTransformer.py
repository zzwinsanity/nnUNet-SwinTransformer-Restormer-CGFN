...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0., pos_embed_method='relative',locked_attn=0):
...
#         self.w = nn.Parameter(torch.diag(torch.rand(int(216))))
        if (self.locked_attn == 1):
#             print('dimdimdimdimdimdimdim ', dim)
#             self.w = nn.Parameter(torch.diag(torch.rand(int(27))))
            self.w = nn.Parameter(torch.diag(torch.rand(int(216))))
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, window_size=(7, 7, 7), shift_size=(0, 0, 0),
                 ...
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm,  pos_embed_method='relative',locked_attn=0):
        super().__init__()
        self.dim = dim
        self.num_heads = num_heads
...
#         print('pad_r, pad_b ', pad_r, pad_b)
        x = nnf.pad(x, (0, 0, pad_l, pad_r, pad_t, pad_b, pad_f, pad_h))
        _, Hp, Wp, Tp, _ = x.shape
        #print("output x size, and the padding" + str(pad_l) +":"+ str(pad_r) + ":" + str(pad_b) + ":"+ str(pad_f) + ":"+ str(pad_h))
#         print(x.size())
        # cyclic shift
